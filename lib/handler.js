...
const DB_STATE_KEY = process.env.DB_STATE_KEY || "quota-query-system";
...
async function readDb() {
  if (useNeonStorage()) {
    ...
    const rows = await sql`SELECT data FROM app_state WHERE key = ${DB_STATE_KEY} LIMIT 1`;
    return normalizeDb(rows[0]?.data || DEFAULT_DB);
  }
  if (useBlobStorage()) {
    try {
  ...
  if (process.env.VERCEL) {
    throw new Error("缺少 DATABASE_URL 或 BLOB_READ_WRITE_TOKEN 环境变量，Vercel 部署需要绑定 Neon 或 Blob 存储");
  }
...
async function writeDb(db) {
  if (useNeonStorage()) {
    ...
    await sql`
      ...
      VALUES (${DB_STATE_KEY}, ${JSON.stringify(normalizeDb(db))}::jsonb, NOW())
      ON CONFLICT (key)
...
async function getNeonSql() {
  if (!neonSqlPromise) {
    neonSqlPromise = import("@neondatabase/serverless").then(({ neon }) => neon(process.env.DATABASE_URL));
  }
...
function normalizeDb(db) {
  return {
    ...
    quotas: Array.isArray(db?.quotas) ? db.quotas : []
  };
}
